...
func TestChunksBasic(t *testing.T) {
	forAllFixtures(t, func(t *testing.T, _ chunk.IndexClient, client chunk.Client) {
		...
		written := []string{}
		for i := 0; i < 5; i++ {
			keys, chunks, _, err := testutils.CreateChunks(s, i, batchSize, model.Now().Add(-time.Hour), model.Now())
		...
		for batch := 0; batch < 50; batch++ {
			...
			for len(chunksToGet) < batchSize {
				...
				ref, err := chunk.ParseExternalKey(userID, key)
				require.NoError(t, err)
				chunksToGet = append(chunksToGet, chunk.LazyChunk{
					ChunkRef:    ref,
					ExternalKey: s.Configs[0].ExternalKey(ref),
				})
			}
			...
			sort.Slice(chunksToGet, func(i, j int) bool {
				return chunksToGet[i].ExternalKey < chunksToGet[j].ExternalKey
			})
			sort.Slice(chunksWeGot, func(i, j int) bool {
				return s.Configs[0].ExternalKey(chunksWeGot[i].ChunkRef) < s.Configs[0].ExternalKey(chunksWeGot[j].ChunkRef)
			})
