...
func (r *Reader) postingsForLabelMatchingV1(ctx context.Context, name string, match func(string) bool) index.Postings {
	e := r.postingsV1[name]
	if len(e) == 0 {
		return index.EmptyPostings()
	}
	...
	for val, offset := range e {
		...
		if err != nil {
			return index.ErrPostings(fmt.Errorf("decode postings: %w", err))
		}
...
func (r *Reader) ShardedPostings(p index.Postings, shardIndex, shardCount uint64) index.Postings {
	...
	for p.Next() {
		...
		if err != nil {
			return index.ErrPostings(fmt.Errorf("series %d not found", id))
		}
...
func (dec *Decoder) PostingsFromDecbuf(d encoding.Decbuf) (int, index.Postings, error) {
	...
	return n, index.NewBigEndianPostings(l), nil
}
